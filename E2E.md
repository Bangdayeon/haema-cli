# haema-cli ↔ SaaS 동작 확인 가이드

CLI 가 실제로 haema 서버에 데이터를 보내고 DB 에 들어가는지 직접 검증하는 시나리오. 위에서 아래로 순서대로 따라가요.
...
| 방식 | 명령 |
|---|---|
| npm 전역 설치 | `npm i -g @haema/cli` 후 `haema <cmd>` |
| 일회성 npx | `npx -p @haema/cli haema <cmd>` |
| 로컬 소스 | `cd /Users/bibi/haema-cli && npm run dev -- <cmd>` |

이 가이드는 `npx -p @haema/cli haema <cmd>` 형태로 적어요.

---

## 1. haema dev 서버 띄우기 (터미널 A)
...
```bash
export HAEMA_API_URL=http://localhost:3000/api/sessions/ingest
export HAEMA_API_KEY=vt_97f2bef211a6f08ae01d386085b25f02503728441b2d99db9d9b3dd3ae31707f
```

- 포트가 3000 아니면 URL 의 `3000` 자리 바꿔요
- `HAEMA_API_KEY` 위 값은 **dev DB 한정** 으로 이전에 발급한 거예요. 키 안 먹으면 새로 발급 (아래 2-A 참고)
...
# 3) 출력 평문 키를 터미널 B 의 HAEMA_API_KEY 로 export
```
...
추가 옵션 (선택):
```bash
npx -p @haema/cli haema inspect --type assistant      # 특정 type 만
npx -p @haema/cli haema inspect --raw --limit 3       # raw JSON 3줄
```
...
```bash
npx -p @haema/cli haema replay --out /tmp/replay.html
...
```bash
npx -p @haema/cli haema replay --serve
```
...
```bash
npx -p @haema/cli haema upload --watch
```
...
**옵션 C — haema 대시보드**: `http://localhost:3000` 접속 → 본인 프로젝트의 세션 detail 페이지에서 새 이벤트 보임.
...
| 에러 | 원인 | 해결 |
|---|---|---|
| `npm error 404 @haema/cli` | npm install 실패 | 인터넷 / 로그인 상태 확인 |
| `401 API 키가 없거나 유효하지 않아요` | `HAEMA_API_KEY` 미설정/오타 | `echo $HAEMA_API_KEY` 확인 후 export 다시 |
| `401` (키 있는데도) | dev DB reset 으로 키가 날아감 | §2-A 로 새로 발급 |
| `ECONNREFUSED 127.0.0.1:3000` | haema dev server 다운 | 터미널 A 에서 `npm run dev` |
| `404 Not Found` + HTML 응답 | haema 가 다른 포트로 떴음 | 터미널 A 출력의 실제 포트로 `HAEMA_API_URL` 수정 |
| `세션 파일을 찾지 못했어요` | 이 cwd 에서 Claude Code 실행 이력 없음 | 다른 cwd 에서 시도 또는 `haema upload ./path.jsonl` 처럼 직접 지정 |
...
성공 신호 세 개만 확인하면 끝:
1. `haema upload` → `업로드 OK · 새 이벤트 N개`
