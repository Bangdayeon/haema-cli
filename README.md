...
`--project` 모드에서는 세션 jsonl 외에 `CLAUDE.md` / `AGENTS.md` / `SKILL.md` 도 자동으로 함께 업로드해요 (`--no-claude-files` 로 비활성 가능).
...
현재 프로젝트의 `CLAUDE.md` / `AGENTS.md` / `SKILL.md` 를 스캔해 votra 서버로 업로드해요. `votra upload --project` 가 자동으로 호출하는 것과 동일한 동작을 단독으로 실행해요.
