...
→ 브라우저 자동 열림 → haema 계정 로그인 → `~/.haema/auth.json` 저장
...
```
설치 → haema install → Claude Desktop 재시작
                    ↓
              signin 툴로 로그인
                    ↓
...
haema 계정에 로그인해요. 브라우저가 자동으로 열리고 로그인 후 `~/.haema/auth.json`에 저장돼요.

| 파라미터 | 타입 | 설명 |
|---------|------|------|
| `appUrl` | string (선택) | haema 서버 URL (기본값 자동 사용) |
...
현재 프로젝트의 `CLAUDE.md` / `AGENTS.md` / `SKILL.md`를 스캔해 haema 서버로 업로드해요.
...
├─ auth.ts                   ~/.haema/auth.json 읽기/쓰기
├─ mcp/
│  ├─ server.ts              MCP 서버 (stdio / HTTP transport)
│  ├─ mcpClient.ts           haema API 클라이언트
